...
// Helper to map frontend 'required' to 'isRequired' and ensure attachment type is valid
const formatPhases = (phases) => {
    if (!phases || !Array.isArray(phases)) return [];
    return phases.map(p => ({
        ...p,
        fields: p.fields?.map(f => ({ ...f, isRequired: f.isRequired ?? f.required ?? false })),
        attachments: p.attachments?.map(a => ({
            ...a,
            type: ['PDF', 'IMAGE', 'file', 'text'].includes(a.type) ? a.type : 'file',
            isRequired: a.isRequired ?? a.required ?? false
        })),
        approvals: p.approvals?.map(a => ({ ...a, isRequired: a.isRequired ?? a.required ?? false })),
        permits: p.permits?.map(pm => ({ ...pm, isRequired: pm.isRequired ?? pm.required ?? false })),
        tasks: p.tasks?.map(t => ({ ...t, isRequired: t.isRequired ?? t.required ?? true }))
    }));
};
...
export const updateProjectType = asynchandler(async (req, res, next) => {
    if (req.body.phases && Array.isArray(req.body.phases)) {
        ...
        req.body.phases = formatPhases(req.body.phases);
    }
